...
本阶段回退基线必须重新捕获为 Web 1.3.0＋两个 Worker 1.2.0，不能使用上面 1.3.0 发布目录的旧回退基线。停写、确认所有任务终态并备份后，回退只恢复本次保存的镜像/配置/latest；继续使用当前数据库，保留新增表、书签和不可变产物供重新升级。旧 Web 不支持 `selection_translate`，不能留下排队或执行中的新类型任务再回退。实际路径与摘要在本次发布完成后补入记录。
