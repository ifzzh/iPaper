...
Web 1.7.0 已发布并部署，已核验镜像摘要及正式验收结果见发布记录。

1.7.0 增加稳定研究主题：本地自动归类、多主题及父级去重浏览、持续人工纠正保护、导入逻辑关联和跨页批量调整。主题可叠加关键词、收藏及 Reading List，论文文件保持原位。用法和限制见[主题说明](docs/development/topics.md)，组件及部署状态见[发布记录](docs/releases/v1.7.0.md)。本轮仅更新 Web，两个 Worker 复用已验证的1.2.0；元数据、关键词、双翻译和阅读工具继续可用。

本地分类使用已采用书目及已有原文，不调用模型、MinerU、OCR或书目服务。定义未覆盖的方向可以人工维护或保留待整理，不强行归类。
