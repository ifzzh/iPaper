...
Web 1.13.7 keeps the note save queue and unresolved conflicts across reader/panel switches, preserves typing during conflict decisions, honours rate-limit waiting without recursion, and revisits the exact PDF or structured-language source of an excerpt. Verification and release availability are recorded in the [release notes](docs/releases/v1.13.7.md). Both Workers remain at 1.2.0.

Web 1.13.2 fixes the V1–V6 findings of the 1.13.1 review: typing during a save and switching panels no longer loses or overwrites newer drafts (the draft always holds the newest text, survives panel switches and resumes saving), conflict decisions really apply to the editor and a failed retry offers actionable buttons (auto-save pauses during a decision and the base revision is not advanced), note writes use a dedicated owner-scoped minute-level limit and honour `Retry-After`, conflict decisions compare the seen revision strictly, and excerpts in the note link back to their source with an explicit degraded path. See the [release record](docs/releases/v1.13.2.md).

Web 1.13.1 makes reading notes trustworthy: saves are bound to the edit version (a slow response cannot replace newer typing), failed drafts survive panel changes with retry and a leave warning, conflict resolution is bound to the revision the user saw and keeps both sides, all four content kinds are wired end to end, annotations can be edited/recoloured/undone, answers and excerpts dedupe by identity, sources are fully revisit-capable with pagination, and a job whose units are already checkpointed can publish without hitting the time budget. See the [release record](docs/releases/v1.13.1.md).
